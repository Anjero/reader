...
import net.anjero.reader.module.system.system.mapper.SysProvinceMapper;
import net.anjero.reader.module.system.system.pojo.SysProvince;
import net.anjero.reader.module.system.system.pojo.SysProvinceExample;
...
public class SysProvinceService {
   ...
   * @return
   */
  public SysProvince selectMunicipalityByName(String name) {
    SysProvinceExample example = new SysProvinceExample();
    example.createCriteria().andNameLike(name + "%").andPidNotEqualTo(1);
    example.setOrderByClause(" id");
    List<SysProvince> list = provinceMapper.selectByExample(example);
    if (list != null && list.size() > 0) return list.get(0);
    return null;
